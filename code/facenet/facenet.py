...
def lppool(inpOp, pnorm, kH, kW, dH, dW, padding):
    ...
    with tf.name_scope('lppool'):
        ...
        subsamp = tf.nn.avg_pool(pwr, ksize=[1, kH, kW, 1],strides=[1, dH, dW, 1],padding=padding,name=name)
...
def mpool(inpOp, kH, kW, dH, dW, padding):
    ...
    with tf.name_scope('maxpool'):
        maxpool = tf.nn.max_pool(inpOp,ksize=[1, kH, kW, 1],strides=[1, dH, dW, 1],padding=padding,name=name)
    return maxpool
def apool(inpOp, kH, kW, dH, dW, padding):
    ...
    name = 'pool' + str(pool_counter)
    pool_counter += 1
    return tf.nn.avg_pool(inpOp,ksize=[1, kH, kW, 1],strides=[1, dH, dW, 1],padding=padding,name=name)
...
def batch_norm(x, n_out, phase_train, name, affine=True):
    ...
    with tf.name_scope(name):
        ...
        beta = tf.Variable(tf.constant(0.0, shape=[n_out]),name=name+'/beta', trainable=True)
        gamma = tf.Variable(tf.constant(1.0, shape=[n_out]),name=name+'/gamma', trainable=affine)
        batch_mean, batch_var = tf.nn.moments(x, [0,1,2], name='moments')
        ema = tf.train.ExponentialMovingAverage(decay=0.9, zero_debias=False)
        ...
        normed = tf.nn.batch_norm_with_global_normalization(x, mean, var,beta, gamma, 1e-3, affine, name=name)
